...
function spinLogoToTop() {
  document.getElementById("fixed_elements__logo-img").className =
    "fixed_elements__logo-img animate__animated animate__rotateIn";
}

function flipInButton() {
  document.getElementById("contact__card__form__button").className =
    "contact__card__form__button animate__animated animate__flipInY animate__faster";
}

function logoHoverFeedback() {
  document.getElementById("fixed_elements__logo-img").className =
    "fixed_elements__logo-img animate__animated animate__pulse animate__faster";
}

document.getElementById("fixed_elements__logo-img").addEventListener("click", spinLogoToTop);

document
  .getElementById("fixed_elements__logo-img")
  .addEventListener("mouseover", logoHoverFeedback);
...
document
  .getElementById("fixed_elements__hamburger-menu")
...
function locked_glink() {
  ...
  filtered_g0 = gfrac_object.gfrac_0_array.filter((i) => {
    return regex_g0.test(i);
  });
  filtered_g1 = gfrac_object.gfrac_1_array.filter((i) => {
    return regex_g1.test(i);
  });
  filtered_g2 = gfrac_object.gfrac_2_array.filter((i) => {
    return regex_g2.test(i);
  });
  filtered_g3 = gfrac_object.gfrac_3_array.filter((i) => {
    return regex_g2.test(i);
  });
  filtered_g4 = gfrac_object.gfrac_4_array.filter((i) => {
    return regex_g1.test(i);
  });
  filtered_g5 = gfrac_object.gfrac_5_array.filter((i) => {
...
function g_displaceLink() {
  ...
  let cvButton2 = document.getElementById("cv2");
  let newHref = g_unlockedLink();
